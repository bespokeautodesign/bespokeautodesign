...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (marineDropdownRef.current && !marineDropdownRef.current.contains(e.target as Node)) {
        setMarineOpen(false);
      }
    };
          ...
          <div className="flex items-center justify-between gap-4 min-w-0">
            ...
            <div className="hidden lg:flex items-center gap-5 xl:gap-8 min-w-0">
              ...
              <div className="relative" ref={dropdownRef} onMouseLeave={() => setPpfOpen(false)}>
                <Link to="/paint-protection-film" onMouseEnter={() => setPpfOpen(true)} className={`flex items-center gap-1 text-sm xl:text-base whitespace-nowrap transition-colors ${isPPFActive || location.pathname === "/paint-protection-film" ? "text-primary font-semibold" : "text-muted-foreground hover:text-primary"}`}>
                  Paint Protection Film
                  <ChevronDown className={`h-3.5 w-3.5 transition-transform ${ppfOpen ? "rotate-180" : ""}`} />
                </Link>
                {ppfOpen && <div className="absolute top-full left-0 pt-2">
                    <div className="w-48 bg-background border border-border rounded-lg shadow-lg z-50 py-2">
                      {ppfSubItems.map(item => <Link key={item.href} to={item.href} onClick={() => setPpfOpen(false)} className={`block px-4 py-2 text-sm transition-colors ${location.pathname === item.href ? "text-primary font-semibold bg-accent" : "text-muted-foreground hover:text-primary hover:bg-accent"}`}>
                          {item.label}
                        </Link>)}
                    </div>
                  </div>}
              ...
              <div className="relative" ref={marineDropdownRef} onMouseLeave={() => setMarineOpen(false)}>
                ...
                {marineOpen && <div className="absolute top-full left-0 pt-2">
                    <div className="w-56 bg-background border border-border rounded-lg shadow-lg z-50 py-2">
                      {marineSubItems.map(item => <Link key={item.href} to={item.href} onClick={() => setMarineOpen(false)} className={`block px-4 py-2 text-sm transition-colors ${location.pathname === item.href ? "text-primary font-semibold bg-accent" : "text-muted-foreground hover:text-primary hover:bg-accent"}`}>
                          {item.label}
                        </Link>)}
                    </div>
                  </div>}
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <Button variant="outline" size="sm" asChild className="hidden xl:flex bg-gradient-to-r from-emerald-500 to-green-600 text-white border-0 shadow-md hover:shadow-lg hover:from-emerald-400 hover:to-green-500 hover:scale-105 transition-all duration-300 font-semibold">
              ...
              <Button variant="outline" size="sm" asChild className="xl:hidden bg-gradient-to-r from-emerald-500 to-green-600 text-white border-0 shadow-md p-2">
